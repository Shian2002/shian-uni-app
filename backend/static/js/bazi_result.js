...
function renderSettingsTab() {
    ...
    html += `<div>玄策台八字排盘 v7.0</div>`;
    html += `<div style="color:var(--text-3);font-size:0.74rem;margin-top:4px;">1:1 复刻问真八字 PC 版排盘结果页功能，保持玄策台视觉风格</div>`;
...
function renderInfoTab(data) {
    ...
    if (wsDetail.detail) {
        ...
        dims.forEach(dim => {
            ...
            html += `<span style="font-size:0.76rem;font-weight:600;color:${status?'#4a8c5c':'#d30505'};">${status?'✓ 得':'✗ 失'}${dim.key==='ling'?'令':dim.key==='di'?'地':dim.key==='sheng'?'生':'助'}</span>`;
            html += `</div>`;
...
function renderProTab(data) {
    ...
    if (tiaohou.yong_shen || tiaohou.tiao_hou) {
        ...
        if (tiaohou.yong_shen) html += `<span class="info-tag">用 ${tiaohou.yong_shen}</span>`;
        if (tiaohou.tiao_hou) html += `<span class="info-tag">候 ${tiaohou.tiao_hou}</span>`;
        if (tiaohou.xi_shen) html += `<span class="info-tag">喜 ${tiaohou.xi_shen}</span>`;
        if (tiaohou.ji_shen) html += `<span class="info-tag">忌 ${tiaohou.ji_shen}</span>`;
        html += `</div></div>`;
    }
    // 格局
    if (gejuDesc) {
...
function _wzSupplementSizhu(szData, dayMaster) {
    ...
    ['year','month','day','hour'].forEach(k => {
        ...
        if (needCompute) {
            const local = wzComputeGanZhiDetail(p.tg + p.dz, dayMaster);
            if (!p.xingyun && local.xingyun) p.xingyun = local.xingyun;
            if (!p.zizuo && local.zizuo) p.zizuo = local.zizuo;
            if (!p.kongwang && local.kongwang) p.kongwang = local.kongwang;
            if ((!p.shensha || !p.shensha.length) && local.shensha && local.shensha.length) p.shensha = local.shensha;
            if (!p.canggan || !p.canggan.length) p.canggan = local.canggan;
            if (!p.ss && local.ss) p.ss = local.ss;
        }
    });
    return szData;
}
...
function _wzRenderLeftTable(data) {
    const sz = _wzSupplementSizhu({...data.sizhu}, data.day_master || '');
...
function _wzRenderGuanxiBox(data) {
    ...
    html += `<div class="wz-guanxi-box-title"><span class="wz-guanxi-box-icon">⚡</span> 干支留意</div>`;
    html += `<div class="wz-guanxi-box-body" style="flex-direction:column;gap:4px;">`;
    if (tgRel) {
        html += `<div class="wz-guanxi-item" style="width:100%;">`;
    ...
    switch(key) {
        ...
        case 'cg': {
            const cgs = (d.canggan || []).map(x => {
                const gan = x.gz || '';
                const ss = x.ss || '';
                return `<span class="wz-canggan-item">${gan ? wxSpanBZ(gan) : ''}<span class="wz-canggan-ss">${ss}</span></span>`;
            }).join('');
            return cgs ? `<span class="wz-canggan-wrap">${cgs}</span>` : '-';
        }
