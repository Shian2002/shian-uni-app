...
- 后端进一步拆出 `backend/ops_routes.py`、`backend/bazi_history_routes.py` 与 `backend/points_routes.py`，降低 `backend/app.py` 路由堆积。
...
- `backend/app.py` 仍然过大，后台管理、健康检查、八字历史、会员积分已经拆出，但社区、充值、排盘、AI、用户体系仍混在主文件里。
