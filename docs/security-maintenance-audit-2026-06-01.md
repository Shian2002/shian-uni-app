...
- 后端进一步拆出 `backend/ops_routes.py`、`backend/auth_routes.py`、`backend/bazi_history_routes.py`、`backend/points_routes.py`、`backend/profile_routes.py` 与 `backend/recharge_routes.py`，降低 `backend/app.py` 路由堆积。
...
- `backend/app.py` 仍然过大，后台管理、基础账号、健康检查、八字历史、会员积分、命盘档案、充值已经拆出，但社区、排盘、AI、第三方/验证码登录仍混在主文件里。
- `src/pages/index/index.vue` 仍然承载首页 AI 对话编排和 artifact 渲染，后续还应继续拆 `useHomeArtifacts` 与综合 AI 请求流控制。

## 下一步建议

1. 继续拆后端：`community_routes.py`、`metaphysics_routes.py`、`comprehensive_routes.py`、第三方/验证码登录路由。
